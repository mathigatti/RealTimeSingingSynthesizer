...
#define OTO_EVENTS_MAX 32
oto_t oto_events[OTO_EVENTS_MAX];
int oto_event_index=0;


void oto_event2(int photype,int silence_sample_count,int unvoiced_sample_count,int voiced_sample_count)
{
    oto_events[oto_event_index].phoneme = photype;
    oto_events[oto_event_index].silence = silence_sample_count;
    oto_events[oto_event_index].unvoiced = unvoiced_sample_count;
    oto_events[oto_event_index].voiced = voiced_sample_count;
    oto_event_index++;
}
#endif
...
int DoSpect2(PHONEME_TAB *this_ph, int which, FMT_PARAMS *fmt_params,  PHONEME_LIST *plist, int modulation)
{
	...
	if (frames == NULL)
    {
        fprintf(stderr,"LookupSpect::NotFound\n");
		return 0; // not found
    }
...
int Generate(PHONEME_LIST *phoneme_list, int *n_ph, bool resume)
{
	...
	if (resume == false) {
		...
		DoPause(0, 0); // isolate from the previous clause (only in singing mode)
        //fprintf(stderr,"not doing pause\n");
	}
	...
	while ((ix < (*n_ph)) && (ix < N_PHONEME_LIST-2)) {
        ...
        //if(sinsy_otoing) DoOto(p->type);//FIXXME
		switch (p->type)
		{
		case phPAUSE:
			DoPause(p->length, 0);
...
int SpeakNextClause(int control)
{
	...
	CalcLengths(translator); //apply preutterance to new note
