...
static void AdvanceParameters()
{
    ...
    if(sinsy_f0) //constant pitch for singing synthesis, as required by tn_fnds
        wdata.pitch = sinsy_f0 * 4096;
...
static int Wavegen()
{
	...
	for (;;) {
        ...
        if(singing_mode)
        {
           sinsy_sample(z,SAMPLE_VOICED);	 //voiced samples have constant pitch
        }
        else
        {
            *out_ptr++ = z;
...
static int PlaySilence(int length, bool resume)
{
	...
	while (n_samples-- > 0) {
        ...
        if(singing_mode)
        {
			sinsy_sample(value,SAMPLE_SILENCE);
		}
        else
        {
			*out_ptr++ = value;
...
static int PlayWave(int length, bool resume, unsigned char *data, int scale, int amp)
{
	...
	while (n_samples-- > 0) {
        ...
        if(singing_mode) {
			sinsy_sample(value,SAMPLE_UNVOICED);	
		}
		else {
			out_ptr[0] = value;
			out_ptr[1] = value >> 8;
...
static int WavegenFill2()
{
	...
	while (out_ptr < out_end) {
		if (WcmdqUsed() <= 0) {
			if (echo_complete > 0) {
                //fprintf(stderr,"PlaySilence in WavegenFill2\n");
		...
		#if 0
        case WCMD_OTO:
            oto_event2(q[1],sinsy_silence_sample_count,sinsy_unvoiced_sample_count,sinsy_voiced_sample_count);
            sinsy_voiced_sample_count = 0;
            sinsy_unvoiced_sample_count = 0;
            sinsy_silence_sample_count = 0;
        break;
        #endif
		case WCMD_PITCH:
